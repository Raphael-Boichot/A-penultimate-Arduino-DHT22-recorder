...
void data_logging() {
  ...
  String Temperature = String(currentTemp, 2);
  String Humidity = String(currentHum, 2);
  ...
  // --- DEGREE HOURS (DH) LOGIC ---

  // Check if a new hour has started to finalize the previous hour's average
  ...
  // Accumulate data for the current hour
  tempSum += currentTemp;
  sampleCount++;
